...
class Database:
    ...
    async def list_owners(self, search: str = "", limit: int = 20, offset: int = 0) -> List[dict]:
        query = """
            SELECT b.owner_id, COUNT(*) as bot_count, MIN(b.created_at) as first_bot,
                   COALESCE(MAX(NULLIF(fu.username, '')), '') as username
            FROM bots b
            LEFT JOIN factory_users fu ON fu.chat_id = b.owner_id
            GROUP BY b.owner_id
        """
        cursor = await self.connection.execute(query)
        rows = await cursor.fetchall()
        result = [{"owner_id": r[0], "bot_count": r[1], "first_bot_at": r[2], "username": r[3]} for r in rows]
        if search:
            result = [
                r for r in result
                if search in str(r["owner_id"]) or search.lower() in r["username"].lower()
            ]
    ...
    async def get_owner_info(self, owner_id: int) -> dict:
        ...
        cursor = await self.connection.execute(
            "SELECT username, first_name FROM factory_users WHERE chat_id = ?", (owner_id,)
        )
        owner_row = await cursor.fetchone()
        return {
            "owner_id": owner_id,
            "username": owner_row[0] if owner_row else "",
            ...
            try:
                ...
                await self.connection.execute(
                    """UPDATE factory_users
                       ...
                       WHERE chat_id = ?""",
                    (username, username, first_name, first_name, chat_id),
                )
                await self.connection.commit()
                return is_new
            except Exception as e:
